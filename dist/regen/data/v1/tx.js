...
exports.MsgAttestResponse = {
    ...
    encode(message, writer = binary_1.BinaryWriter.create()) {
        for (const v of message.newEntries) {
            types_1.AttestorEntry.encode(v, writer.uint32(10).fork()).ldelim();
        }
        return writer;
    },
    decode(input, length) {
        ...
        while (reader.pos < end) {
            ...
            switch (tag >>> 3) {
                case 1:
                    message.newEntries.push(types_1.AttestorEntry.decode(reader, reader.uint32()));
                    break;
                default:
                    reader.skipType(tag & 7);
                    break;
    ...
    fromPartial(object) {
        const message = createBaseMsgAttestResponse();
        message.newEntries = object.newEntries?.map(e => types_1.AttestorEntry.fromPartial(e)) || [];
        return message;
    },
    fromAmino(object) {
        const message = createBaseMsgAttestResponse();
        message.newEntries = object.new_entries?.map(e => types_1.AttestorEntry.fromAmino(e)) || [];
        return message;
    },
    toAmino(message) {
        const obj = {};
        if (message.newEntries) {
            obj.new_entries = message.newEntries.map(e => e ? types_1.AttestorEntry.toAmino(e) : undefined);
        }
        else {
